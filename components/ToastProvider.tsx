...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getToastStyles = (type: ToastType) => {
    switch (type) {
      ...
      case 'info':
        return {
          bg: 'bg-gradient-to-r from-emerald-800 to-slate-900',
          icon: Info,
          iconColor: 'text-white',
        };
    }
      ...
      <div className="fixed top-4 right-4 z-[9999] space-y-3 pointer-events-none">
        {toasts.map((toast) => {
          ...
          return (
            <div
              key={toast.id}
              className={`${styles.bg} text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 min-w-[320px] max-w-md pointer-events-auto animate-slideInRight border border-white/10`}
              style={{
